...
Executable = $ENV(CMSSW_BASE)/src/TopTagTest/test/condor/goMakePlots.sh
Requirements = OpSys == "LINUX"&& (Arch != "DUMMY" )
Should_Transfer_Files = YES
WhenToTransferOutput = ON_EXIT
Transfer_Input_Files = $ENV(CMSSW_BASE)/src/TopTagTest/test/remnantSys, $ENV(CMSSW_BASE)/src/TopTagTest/test/condor/goMakePlots.sh, $ENV(CMSSW_BASE)/lib/$ENV(SCRAM_ARCH)/librecipeAUXOxbridgeMT2.so, $ENV(CMSSW_BASE)/lib/$ENV(SCRAM_ARCH)/libTopTaggerCfgParser.so, $ENV(CMSSW_BASE)/lib/$ENV(SCRAM_ARCH)/libTopTaggerTopTagger.so, $ENV(CMSSW_BASE)/src/TopTagger/TopTagger/test/Example_TopTagger.cfg
    ...
    for s, n in sc.sampleList(ds):
        ...
        if not f == None:
            ...
            for startFileNum in xrange(0, count, nFilesPerJob):
                ...
                fileParts.append("Output = logs/remnantSys_%s_%i.stdout\n"%(n, startFileNum/nFilesPerJob))
                fileParts.append("Error = logs/remnantSys_%s_%i.stderr\n"%(n, startFileNum/nFilesPerJob))
                fileParts.append("Log = logs/remnantSys_%s_%i.log\n"%(n, startFileNum/nFilesPerJob))
